document.addEventListener('DOMContentLoaded', function () {
    ...
    // Añadir asignaturas a sus respectivas columnas
    ...
    const todosLosDivs = document.querySelectorAll('.asignatura');

    todosLosDivs.forEach(div => {
        div.addEventListener('mouseover', () => {
            const asignaturaId = div.dataset.id;
            resaltarRutas(asignaturaId);
            container.classList.add('filtrando');
        });

        div.addEventListener('mouseout', () => {
            limpiarResaltado();
            container.classList.remove('filtrando');
        });
    });

    function resaltarRutas(asignaturaId) {
        limpiarResaltado();
        const asignaturaActual = findAsignaturaById(asignaturaId);
        if (!asignaturaActual) return;

        const elActual = document.getElementById(asignaturaId);
        if (elActual) elActual.classList.add('resaltado-base');

        // Resaltar prerrequisitos
        const prerrequisitos = getAllPrerequisitos(asignaturaId);
        prerrequisitos.forEach(id => {
            document.getElementById(id)?.classList.add('resaltado-prerequisito');
        });

        // Resaltar consecuencias (ramos que dependen de este)
        const consecuencias = getAllConsecuencias(asignaturaId);
        consecuencias.forEach(id => {
            document.getElementById(id)?.classList.add('resaltado-consecuencia');
        });
    }

    function limpiarResaltado() {
        todosLosDivs.forEach(div => {
            div.classList.remove('resaltado-base', 'resaltado-prerequisito', 'resaltado-consecuencia');
        });
    }

    function findAsignaturaById(id) {
        return asignaturas.find(a => a.id === id);
    }

    function getAllPrerequisitos(id, lista = new Set()) {
        const asignatura = findAsignaturaById(id);
        if (asignatura && asignatura.prerequisitos.length > 0) {
            asignatura.prerequisitos.forEach(prereqId => {
                if (!lista.has(prereqId)) {
                    lista.add(prereqId);
                    getAllPrerequisitos(prereqId, lista);
                }
            });
        }
        return lista;
    ...
    function getAllConsecuencias(id, lista = new Set()) {
        const consecuenciasDirectas = asignaturas.filter(a => a.prerequisitos.includes(id));
        consecuenciasDirectas.forEach(consecuencia => {
            if (!lista.has(consecuencia.id)) {
                lista.add(consecuencia.id);
                getAllConsecuencias(consecuencia.id, lista);
            }
        });
        return lista;
    }
